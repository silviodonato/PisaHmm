#!/bin/bash
ZREGION=LeadMuon_pt___ZRegion
ZCONTROL=LeadMuon_pt___ZRegion
SIGNAL=LeadMuon_pt___SignalRegion
...
combine  -M MultiDimFit -n$1 --saveWorkspace --setParameters mask_ch1=1,mask_ch2_${SIGNAL}=1,mask_ch2_${SIDE}=0 datacard${1}.root --verbose 9   --robustFit 1 >>${1}.log

combine -M Significance --snapshotName MultiDimFit -t -1  higgsCombine${1}.MultiDimFit.mH120.root --setParameters mask_ch1=1,mask_ch2_${SIGNAL}=0,mask_ch2_${SIDE}=0,r=1   >>${1}.log
combine -M Significance --snapshotName MultiDimFit -t -1  higgsCombine${1}.MultiDimFit.mH120.root --setParameters mask_ch1=1,mask_ch2_${SIGNAL}=0,mask_ch2_${SIDE}=0,r=1  --toysFrequentist  >>${1}.log
...
DC=higgsCombine${1}.MultiDimFit.mH120.root
combineTool.py -M Impacts -d $DC -m 125 -n$1 --setParameters mask_ch1=1,mask_ch2_${SIGNAL}=0,mask_ch2_${SIDE}=0,r=1 --doInitialFit --robustFit 1  >${1}.impact.log
combineTool.py -M Impacts -d $DC -m 125 -n$1 --setParameters mask_ch1=1,mask_ch2_${SIGNAL}=0,mask_ch2_${SIDE}=0,r=1 --robustFit 1 --doFits   --parallel 50 >>${1}.impact.log
combineTool.py -M Impacts -d $DC -m 125 -n$1 -o impacts${1}.json  >>${1}.impact.log


 
combineTool.py -M FitDiagnostics -m 125 -d datacard${1}.root --there --cminDefaultMinimizerStrategy 0 --saveWorkspace --setParameters mask_ch1=1,mask_ch2_${SIGNAL}=1,mask_ch2_${SIDE}=0,r=1 
...
python ./postFitPlot.py --file=shapes${1}.root --ratio --extra_pad=0.43   --ratio_range 0.4,1.6 --empty_bin_error --channel=ch1 --outname ${ZCONTROL}  --mode postfit --log_y --custom_y_range --y_axis_min "1E+1"  --channel_label "VBF Hmm" --file_dir ch1_postfit
...
python ./postFitPlot.py --file=shapes${1}.root --ratio --extra_pad=0.43   --ratio_range 0.4,1.6 --empty_bin_error --channel=ch1 --outname ${ZCONTROL}  --mode prefit --log_y --custom_y_range --y_axis_min "1E+1"  --channel_label "VBF Hmm" --file_dir ch1_prefit
